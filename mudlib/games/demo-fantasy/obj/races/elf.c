...
void setup()
{
  ...
  set_body_size(5);
  set_name(_LANG_RACES_ELF_NAME);
  set_short(capitalize(_LANG_RACES_ELF_NAME));
  set_long(_LANG_RACES_ELF_DESC);  
...
string query_desc(object ob)
{
  if (ob)
  {
    if ((int)ob->query_gender() == 1)
      return _LANG_RACES_ELF_DESC_MALE;
    return _LANG_RACES_ELF_DESC_FEMALE;
  }
  return "";
}
...
{
  if (!flag)
    return player->query_article() + " " + _LANG_RACES_ELF_GENDER_STRING;
  else
    return _LANG_RACES_ELF_GENDER_STRING;
}
