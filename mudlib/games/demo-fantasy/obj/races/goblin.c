...
string query_desc(object ob)
{
  if (ob)
  {
    if ((int)ob->query_gender() == 1)
      return _LANG_RACES_GOBLIN_DESC_MALE;
    return _LANG_RACES_GOBLIN_DESC_FEMALE;
  }
  return "";
}
...
{
  if (!flag)
    return player->query_article() + " " + _LANG_RACES_GOBLIN_GENDER_STRING;
  else
    return _LANG_RACES_GOBLIN_GENDER_STRING;
}
