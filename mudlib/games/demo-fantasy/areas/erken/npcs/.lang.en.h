
#define _LANG_NPCS_GOBLIN_NAME "goblin"
#define _LANG_NPCS_GOBLIN_CHAT ({ "'Erken is ours!", \
                                  ":smiles maliciously." })
#define _LANG_NPCS_GOBLIN_A_CHAT ({ "'Grrrrrrrr!!!", \
                                    "'I'm gonna bite your ear!" })

#define _LANG_NPCS_GNOLL_NAME "gnoll"
#define _LANG_NPCS_GNOLL_LONG "Similar to a goblin, but thinner and taller, and with its typical " + \
    "long nose. It's a disgusting thing.\n"
#define _LANG_NPCS_GNOLL_A_CHAT ({ "'Grrrrrrrr!!!", \
                                   "'Urrrrl die!", })

#define _LANG_NPCS_ORC_NAME "orc"
#define _LANG_NPCS_ORC_LONG "Un big humanoid of greenish skin, strong and menacing.\n"
#define _LANG_NPCS_ORC_CHAT ({ ":grunts.", \
                               ":spits.", \
                               ":curses your name.", \
                               ":looks at you maliciously with $oposs$ bloodshot eyes.", })

#define _LANG_NPCS_BARMAN_NAME "arak"
// "barman" will be use by set_attender in the pub, do not change!
#define _LANG_NPCS_BARMAN_ALIAS ({ "arak", "barman" })
#define _LANG_NPCS_BARMAN_PLURAL "arak"
#define _LANG_NPCS_BARMAN_PLURALS ({ "arak" })
#define _LANG_NPCS_BARMAN_LONG "For a goblin he is quite remarkable in stature. The smell of him is nauseating.\n"
#define _LANG_NPCS_BARMAN_CHAT ({ ":looks at you.", \
                                  ":spits at your feet.", })
#define _LANG_NPCS_BARMAN_A_CHAT ({ "'Arrggghhh!" })

#define _LANG_NPCS_ASSASSIN_NAME "kael"
#define _LANG_NPCS_ASSASSIN_ALIAS ({ "Kael", "assassin" })
#define _LANG_NPCS_ASSASSIN_MAIN_PLURAL "Assassins"
#define _LANG_NPCS_ASSASSIN_PLURALS ({ "assassins" })
#define _LANG_NPCS_ASSASSIN_SHORT "Kael, the Assassin"
#define _LANG_NPCS_ASSASSIN_LONG "Kael is a dark, shadowy figure dressed in all black. " + \
                    "He moves silently and appears to be assessing your every move.\n"

#define _LANG_NPCS_SHOPKEEPER_NAME "grukk"
// "shopkeeper" will be used by set_attender in the shop, do not change!
#define _LANG_NPCS_SHOPKEEPER_ALIAS ({ "grukk", "shopkeeper" })
#define _LANG_NPCS_SHOPKEEPER_PLURAL "grukk"
#define _LANG_NPCS_SHOPKEEPER_PLURALS ({ "grukk" })
#define _LANG_NPCS_SHOPKEEPER_LONG "A scrawny goblin sitting on a heap of things that do not " + \
      "match one another. Many still carry somebody else's initials, and he is not going " + \
      "to discuss that.\n"

#define _LANG_NPCS_SHOPKEEPER_CHAT ({ "'Cheap, cheap. Don't ask where from.", \
                                      ":counts his coins again, just in case.", \
                                      "'Nobody gets a refund here." })
#define _LANG_NPCS_SHOPKEEPER_A_CHAT ({ "'Mine! All mine!", \
                                        "'Arak! Help!" })
