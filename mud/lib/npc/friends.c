...
void pile_in(object ob,object ob1)
{
	...
	{
		if ( !this_object()->query_attacker_list() || member_array(ob, this_object()->query_attacker_list()) == -1)
		{
	...
	if (ob != this_object() && ob1 != this_object() &&  sizeof(this_object()->query_protect()) && !this_object()->query_protecting() )
	{
		if (!interactive(ob) && member_array(ob->query_name(), this_object()->query_protect()) != -1)
		{
			tell_room(environment(this_object()), capitalize(this_object()->query_short()) + 
			    " se une a la batalla y protege a "+
			ob->query_short()+".\n");
